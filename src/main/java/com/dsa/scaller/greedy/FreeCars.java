...
public class FreeCars {
    ...
    public int solve(int[] A, int[] B) {
        ...
        int time = 0;
        for (int i = 0; i < A.length; i++) {
            int currentTime = jobs.get(i).startTime - 1;

            if (time <= currentTime) {
                queue.add(jobs.get(i).endTime);
                time++;
            } else {
                if (queue.peek() < jobs.get(i).endTime) {
                    queue.poll();
                    queue.add(jobs.get(i).endTime);
                }
            }
    ...
    public static void main(String[] args) {
        ...
        freeCars.solve(new int[]{1, 7, 6, 2, 8, 4, 4, 6, 8, 2}, new int[]{8, 11, 7, 7, 10, 8, 7, 5, 4, 9});
    }
}
